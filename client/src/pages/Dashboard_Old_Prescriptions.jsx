...
export default function Dashboard() {
  ...
  const [analyzingSlip, setAnalyzingSlip] = useState(null);
  ...
  async function analyzeSlip(slipId) {
    setAnalyzingSlip(slipId);
    try {
      const { data } = await api.post(`/slips/${slipId}/analyze`);
      // Update the slip in the list
      setSlips(slips.map((s) => (s._id === slipId ? data.slip : s)));
      if (selectedSlip && selectedSlip._id === slipId) {
        setSelectedSlip(data.slip);
      }
    } catch (err) {
      alert(err.response?.data?.error || "Failed to analyze prescription");
    } finally {
      setAnalyzingSlip(null);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        {/* AI Health Report - Only show if there are prescriptions */}
        {totalPrescriptions > 0 && <AIHealthReport user={user} />}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="border-t border-gray-200 pt-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {!selectedSlip.aiAnalysis && (
                    <button
                      onClick={() => analyzeSlip(selectedSlip._id)}
                      disabled={analyzingSlip === selectedSlip._id}
                      className="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:shadow-lg transition-all duration-200 disabled:opacity-50 flex items-center space-x-2"
                    >
                      {analyzingSlip === selectedSlip._id ? (
                        <>
                          <svg
                ...
                {selectedSlip.aiAnalysis ? (
                  <div className="space-y-4">
                    ...
                    <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl p-4">
                      ...
                      <p className="text-gray-700 text-sm">
                        {selectedSlip.aiAnalysis.summary}
                      </p>
                    </div>

                    {/* Health Issues */}
                    {selectedSlip.aiAnalysis.healthIssues?.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-gray-900 mb-2">
                          Health Issues Identified
                        </h4>
                        <ul className="space-y-1">
                          {selectedSlip.aiAnalysis.healthIssues.map((issue, idx) => (
                    ...
                    {/* Causes */}
                    {selectedSlip.aiAnalysis.causes?.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-gray-900 mb-2">Possible Causes</h4>
                        <ul className="space-y-1">
                          {selectedSlip.aiAnalysis.causes.map((cause, idx) => (
                            <li key={idx} className="flex items-start space-x-2 text-sm">
                    ...
                    {/* Precautions */}
                    {selectedSlip.aiAnalysis.precautions?.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-gray-900 mb-2">Precautions</h4>
                        <ul className="space-y-1">
                          {selectedSlip.aiAnalysis.precautions.map((precaution, idx) => (
                            <li key={idx} className="flex items-start space-x-2 text-sm">
                    ...
                    {/* Remedies */}
                    {selectedSlip.aiAnalysis.remedies?.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-gray-900 mb-2">Remedies</h4>
                        <ul className="space-y-1">
                          {selectedSlip.aiAnalysis.remedies.map((remedy, idx) => (
                            <li key={idx} className="flex items-start space-x-2 text-sm">
                    ...
                    {/* Recommendations */}
                    {selectedSlip.aiAnalysis.recommendations?.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-gray-900 mb-2">Recommendations</h4>
                        <ul className="space-y-1">
                          {selectedSlip.aiAnalysis.recommendations.map((rec, idx) => (
                            <li key={idx} className="flex items-start space-x-2 text-sm">
                    ...
                    <p className="text-xs text-gray-500 italic pt-2">
                      Analyzed on {new Date(selectedSlip.aiAnalysis.analyzedAt).toLocaleString()}
                    </p>
                  </div>
                ) : (
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p>Click "Analyze with AI" to get health insights</p>
                  </div>
                )}
              </div>
